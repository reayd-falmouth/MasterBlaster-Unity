...
public class BMFont_To_LegacyFont_Importer : EditorWindow
{
    ...
    public static void ShowWindow() => GetWindow<BMFont_To_LegacyFont_Importer>(false, "BMFont → Legacy Font");
    ...
    void OnGUI()
    {
        ...
        fntFile      = (TextAsset)EditorGUILayout.ObjectField("BMFont .fnt", fntFile, typeof(TextAsset), false);
        atlasTexture = (Texture2D)EditorGUILayout.ObjectField("Atlas Texture (.png)", atlasTexture, typeof(Texture2D), false);
        targetFont   = (Font)EditorGUILayout.ObjectField("Target Font (Legacy)", targetFont, typeof(Font), false);
        flipY        = EditorGUILayout.ToggleLeft("Flip Y (BMFont top‑left → UV bottom‑left)", flipY);
        createMaterialIfMissing = EditorGUILayout.ToggleLeft("Create/Update Font Material", createMaterialIfMissing);
        ...
        using (new EditorGUI.DisabledScope(fntFile == null || atlasTexture == null))
        {
            ...
            {
                try { Import(); }
                catch (Exception e) { Debug.LogError(e); EditorUtility.DisplayDialog("BMFont Importer", e.Message, "OK"); }
            }
        }

        EditorGUILayout.HelpBox("After import, assign this Font to a standard UI Text component. Keep the Text object's scale at (1,1,1) and scale via Font Size.", MessageType.Info);
    }

    class BMCommon { public int lineHeight, baseLine, scaleW, scaleH; }
    class BMChar   { public int id, x, y, width, height, xoffset, yoffset, xadvance; }

    void Import()
    {
        // Parse .fnt
        if (!TryParseFNT(fntFile, out BMCommon common, out List<BMChar> chars))
            throw new Exception("Could not parse .fnt file. Make sure it is an AngelCode BMFont (.fnt) in text or XML format.");
        ...
        if (!targetFont)
        {
            ...
            var fontPath = AssetDatabase.GenerateUniqueAssetPath(Path.Combine(dir, name + ".fontsettings"));
        ...
        foreach (var c in chars) byId[c.id] = c; // last wins
        ...
        if (lineProp != null) lineProp.floatValue = Mathf.Max(1, common.lineHeight);
        so.ApplyModifiedProperties();
        ...
        EditorUtility.DisplayDialog("BMFont Importer", $"Imported {infos.Length} glyphs into '{targetFont.name}'.\nAssign this Font to a standard UI Text component.", "OK");
    ...
    static bool TryParseFNT(TextAsset fnt, out BMCommon common, out List<BMChar> chars)
    {
        ...
        if (txt.StartsWith("<")) return ParseXML(txt, out common, out chars);
    ...
    static bool ParseXML(string xml, out BMCommon common, out List<BMChar> chars)
    {
        ...
        try
        {
            ...
            common.lineHeight = int.Parse(n.Attributes["lineHeight"].Value, CultureInfo.InvariantCulture);
        ...
        catch (Exception e) { Debug.LogError("BMFont XML parse error: " + e.Message); return false; }
    }
    ...
    static bool ParseText(string text, out BMCommon common, out List<BMChar> chars)
    {
            ...
            while ((line = sr.ReadLine()) != null)
            {
                if (line.StartsWith("common "))
                {
                    foreach (var part in line.Split(' '))
                    {
                        ...
                        if (kv[0] == "lineHeight") common.lineHeight = int.Parse(kv[1]);
                        if (kv[0] == "base")       common.baseLine   = int.Parse(kv[1]);
                        if (kv[0] == "scaleW")     common.scaleW     = int.Parse(kv[1]);
                        if (kv[0] == "scaleH")     common.scaleH     = int.Parse(kv[1]);
                    }
                }
                else if (line.StartsWith("char "))
                {
                    ...
                    foreach (var part in line.Split(' '))
                    {
                        ...
                        switch (kv[0])
                        {
                            case "id":       ch.id = int.Parse(kv[1]); break;
                            case "x":        ch.x = int.Parse(kv[1]); break;
                            case "y":        ch.y = int.Parse(kv[1]); break;
                            case "width":    ch.width = int.Parse(kv[1]); break;
                            case "height":   ch.height = int.Parse(kv[1]); break;
                            case "xoffset":  ch.xoffset = int.Parse(kv[1]); break;
                            case "yoffset":  ch.yoffset = int.Parse(kv[1]); break;
                            case "xadvance": ch.xadvance = int.Parse(kv[1]); break;
                        }
                    }
                    chars.Add(ch);
                }
        ...
        catch (Exception e) { Debug.LogError("BMFont text parse error: " + e.Message); return false; }
    }
}
