...
public class SceneFlowMapperTests
{
    ...
    {
        Assert.That(SceneFlowMapper.StateForSceneName("Credits", _config), Is.EqualTo(FlowState.Credits));
        Assert.That(SceneFlowMapper.StateForSceneName("Title", _config), Is.EqualTo(FlowState.Title));
        Assert.That(SceneFlowMapper.StateForSceneName("Menu", _config), Is.EqualTo(FlowState.Menu));
        Assert.That(SceneFlowMapper.StateForSceneName("Countdown", _config), Is.EqualTo(FlowState.Countdown));
        Assert.That(SceneFlowMapper.StateForSceneName("Game", _config), Is.EqualTo(FlowState.Game));
        Assert.That(SceneFlowMapper.StateForSceneName("Standings", _config), Is.EqualTo(FlowState.Standings));
        Assert.That(SceneFlowMapper.StateForSceneName("Wheel", _config), Is.EqualTo(FlowState.Wheel));
        Assert.That(SceneFlowMapper.StateForSceneName("Shop", _config), Is.EqualTo(FlowState.Shop));
        Assert.That(SceneFlowMapper.StateForSceneName("Overs", _config), Is.EqualTo(FlowState.Overs));
    }

    [Test]
    public void StateForSceneName_UnknownSceneName_ReturnsMenu()
    {
        Assert.That(SceneFlowMapper.StateForSceneName("UnknownScene", _config), Is.EqualTo(FlowState.Menu));
    }
    ...
    public void SceneFor_WithDefaultConfig_ReturnsCorrectSceneNameForEachState()
    {
        ...
        Assert.That(SceneFlowMapper.SceneFor(FlowState.Countdown, _config), Is.EqualTo("Countdown"));
        Assert.That(SceneFlowMapper.SceneFor(FlowState.Game, _config), Is.EqualTo("Game"));
        Assert.That(SceneFlowMapper.SceneFor(FlowState.Standings, _config), Is.EqualTo("Standings"));
    ...
    public void StateForSceneName_And_SceneFor_AreInverse_ForAllStates()
    {
        foreach (FlowState state in System.Enum.GetValues(typeof(FlowState)))
        {
            ...
            Assert.That(SceneFlowMapper.StateForSceneName(sceneName, _config), Is.EqualTo(state),
