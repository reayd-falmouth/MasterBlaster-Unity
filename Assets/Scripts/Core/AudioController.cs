    ...
    public class AudioController : PersistentSingleton<AudioController>
    {
        ...
        private void OnSceneLoaded(Scene scene, LoadSceneMode mode)
        {
            if (scene.name == "Game")
            {
                ...
                {
                    Debug.LogWarning("[AudioController] No 'Game' entry in sceneMusicMap — arena will be silent.");
                    CancelLoop();
                    StopAllMusicSources();
                    return;
                }
        ...
        public void PlayArenaMusic()
        {
            ...
            Debug.Log($"[ArenaMusic] play={_arenaMusicPlayCount} pitch={pitch:F2}");
        ...
        public void PlayArenaOutro(float crossfadeSeconds = 0f)
        {
            ...
            if (clip == null)
            {
                Debug.LogWarning("[AudioController] PlayArenaOutro called but no outroClip is set on the Game scene entry.");
                return;
            }
            float pitch = _arenaEntry.pitch > 0f ? _arenaEntry.pitch : 1f;
