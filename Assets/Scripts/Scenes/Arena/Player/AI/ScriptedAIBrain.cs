...
namespace Scenes.Arena.Player.AI
{
    ...
    public class ScriptedAIBrain : MonoBehaviour, IAIBrain
    {
        ...
        )
        {
            ...
            {
                float dist = Vector2.Distance(myPos, targetPlayer.transform.position);
            ...
            if (item != null)
            {
                Vector2 toItem = (Vector2)item.transform.position - myPos;
            ...
            {
                Vector2 toTarget = (Vector2)targetPlayer.transform.position - myPos;
        ...
        private bool IsCellInDanger(Vector2 cell)
        {
            ...
            {
                if (b == null) continue;
                Vector2 bombCell = RoundToCell(b.transform.position);
        ...
        {
            if (destructibleTiles != null && destructibleTiles.GetTile(destructibleTiles.WorldToCell(cell)) != null)
                return false;
            return true;
        }
        ...
        private ItemPickup GetNearestItem(Vector2 myPos)
        {
            ...
            foreach (var item in items)
            {
                ...
                float sqr = ((Vector2)item.transform.position - myPos).sqrMagnitude;
                if (sqr < best)
                {
                    best = sqr;
                    nearest = item;
